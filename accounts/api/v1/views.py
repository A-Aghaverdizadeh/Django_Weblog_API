...
class ResendActivateUserAPIView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        EmailThread(email_obj).run()
        return Response(
            {"details": "we resend the activation link"},
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user:
            ...
            email_obj = EmailMessage(
                ...
                {"reset_url_token": reset_url_token},
                "admin@gmail.com",
                to=[email],
            )
            EmailThread(email_obj).run()
            ...
            return Response(
                {
                    "details": f"we send an email for {email}",
                    "user token": token,
                },
                status=status.HTTP_200_OK,
            )
            ...
            return Response(
                {
                    "details": "User with this creadantials not found",
                    "email": email,
                    "user": user,
                },
...
class PasswordResetAPIView(GenericAPIView):
    ...
    def post(self, request, token, *args, **kwargs):
        ...
        if user:
            ...
            return Response(
                {"success": "Password Rest confirmed"},
